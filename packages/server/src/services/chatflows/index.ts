...
const updateChatflow = async (chatflow: ChatFlow, updateChatFlow: ChatFlow): Promise<any> => {
    try {
        ...
        // Save a version of the flow after successful database update
        await saveFlowVersion(dbResponse);

        return dbResponse
    } catch (error) {
        if (getErrorMessage(error).includes('[Audit Trail]')) {
             logger.error(`[Audit Trail] Failed during updateChatflow save operation for ${chatflow.id}: ${getErrorMessage(error)}`);
        }
        throw new InternalFlowiseError(
            StatusCodes.INTERNAL_SERVER_ERROR,
